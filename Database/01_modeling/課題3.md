...
LEFT JOIN MenuItem mi ON oi.menu_item_id = mi.id  -- MenuItem テーブルの結合を追加
LEFT JOIN TimeSaleMenuCategory tsmc ON mi.menu_category_id = tsmc.menu_category_id  -- MenuItemのカテゴリーIDを使用
LEFT JOIN TimeSale ts ON tsmc.time_sale_id = ts.id
    AND ts.active = TRUE
    AND NOW() BETWEEN ts.start_time AND ts.end_time  -- 現在時刻での有効なタイムセールのみを対象
WHERE o.id = 1
